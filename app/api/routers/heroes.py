...
async def create_hero(*, session: SessionDep, hero: HeroCreate) -> Hero:
...
async def read_hero(*, session: SessionDep, hero_id: uuid.UUID) -> Hero:
...
async def update_hero(
    *, session: SessionDep, hero_id: uuid.UUID, hero: HeroUpdate
) -> Hero:
...
async def assign_hero_to_team(
    *, session: SessionDep, hero_id: uuid.UUID, team_id: uuid.UUID
) -> Hero:
    hero = await session.get(Hero, hero_id)
    if not hero:
        raise HTTPException(
...
async def delete_hero(*, session: SessionDep, hero_id: uuid.UUID) -> None:
    hero = await session.get(Hero, hero_id)
    if not hero:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Hero not found"
