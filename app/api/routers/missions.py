...
async def create_mission(*, session: SessionDep, mission: MissionCreate) -> Mission:
...
async def assign_hero_to_mission(
    *, session: SessionDep, mission_id: uuid.UUID, hero_id: uuid.UUID
) -> Mission:
...
async def read_mission(*, session: SessionDep, mission_id: uuid.UUID) -> Mission:
    mission = await session.get(Mission, mission_id)
    if not mission:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Mission not found"
...
async def update_mission(
    *, session: SessionDep, mission_id: uuid.UUID, mission: MissionUpdate
) -> Mission:
